...
NA - Not Available, this value appears when data is missing or undefined.    
...
This suggests that there might be some cases that contain significantly large values. since the mean is more sensitive to extremes cases this explains why the mean is significantly greater then the median.
...
visitors_with_nations %>%
  filter(nation == "Scotland") %>%
    summarise(
      Total_In_2021 = sum(n_2021, na.rm = TRUE),
      Total_In_2022 = sum(n_2022),
      percentage_change = (Total_In_2022/Total_In_2021)*100
    )
